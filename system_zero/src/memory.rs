...
pub(crate) fn eval_memory<F: Field, P: PackedField<Scalar = F>>(
    ...
    yield_constr.constraint(context_range_check - trace_context * (next_addr_segment - addr_segment) - (F::ONE - trace_context) * (next_addr_context - addr_context - F::ONE));
    yield_constr.constraint(segment_range_check - trace_segment * (next_addr_virtual - addr_virtual) - (F::ONE - trace_segment) * (next_addr_segment - addr_segment - F::ONE));
    yield_constr.constraint(virtual_range_check - trace_virtual * (next_timestamp - timestamp) - (F::ONE - trace_virtual) * (next_addr_virtual - addr_virtual - F::ONE));
    ...
    yield_constr.constraint(next_current - (F::ONE - trace_context) * (F::ONE - trace_segment) * (F::ONE - trace_virtual) * to);
